...
<template>
  <div class="tool-bar" :style="toolBarStyle">
    ...
    <template v-for="(type, typeIndex) in Object.keys(sysMap)">
      <ToolBox mode="horizontal" :key="typeIndex" :class="type" style="z-index: 93">
        <template v-for="(item, index) in sysMap[type]">
          <ToolItem
            v-if="item.type === 'text'"
            :key="'tool_' + type + '_item_sys' + index"
            :title="$t(item.lang)"
            :active="item.active"
          ...
          <XDivider
            ...
            :key="'tool_' + type + '_divider_sys' + index"
            mode="vertical"
          />
        </template>
      </ToolBox>
    ...
    <template v-for="(type, typeIndex) in Object.keys(toolMap)">
      <ToolBox mode="horizontal" :key="typeIndex" :class="type" style="z-index: 92">
    ...
    <template v-for="(type, typeIndex) in Object.keys(functionMap)">
      <ToolBox mode="horizontal" :key="typeIndex" :class="type" style="z-index: 91">
        <template v-for="(item, index) in functionMap[type]">
          <ToolItem
            v-if="item.type === 'text'"
            :key="'tool_' + type + '_item_function' + index"
            :title="$t(item.lang)"
            :active="item.active"
            :disabled="item.disabled"
            :style="item.toolbar.style"
          ...
          <XDivider
            class="divider"
            v-if="item.toolbar.divider"
            :key="'tool_' + type + '_divider_function' + index"
            mode="vertical"
          />
        </template>
      </ToolBox>
    ...
    sysMap() {
      ...
      _t.sysList.forEach(item => {
        if (item.enable && item.toolbar && item.toolbar.enable) {
          let position = 'left' || item.toolbar.position
