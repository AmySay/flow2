...
<template>
  <div class="materials-editor" @click="handleEditorClick">
    ...
    <PanelRight :originData='originData' :eventItem='eventItem'></PanelRight>
  ...
  export default {
    ...
    methods: {
      ...
      doEditorClick (info) {
        ...
        if (localStorage.getItem('originData' + String(id))) {
          this.originData = JSON.parse(localStorage.getItem('originData' + String(id)))
        } else {
          getSvgById(id).then(res => {
            this.originData = res.data
            localStorage.setItem('originData' + String(id), JSON.stringify(res.data))
          })
        }
      },
      doSetMode (name) {
